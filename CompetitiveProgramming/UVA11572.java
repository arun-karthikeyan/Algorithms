...
public static void main(String[] args) throws Exception {
	...
	while (testcases-- > 0) {
		int n = readInt();
		AVLTreeBasic<Integer> snowFlakes = new AVLTreeBasic<Integer>();
		int maxSize = -1, start = 0, end = 0;
		int[] indices = new int[n];
		while(n-->0){
			int snowFlake = readInt();
			if(snowFlakes.contains(snowFlake)){
				maxSize = max(maxSize, end-start);
				int toRemove;
				while((toRemove = indices[start++])!=snowFlake){
					snowFlakes.delete(toRemove);
				}
				indices[end++] = snowFlake;
			}else{
				indices[end++] = snowFlake;
