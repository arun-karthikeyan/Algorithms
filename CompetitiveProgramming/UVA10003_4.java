...
/**
 * Cutting Sticks - Bottom Up Approach with Knuth Optimization using Quadrangle
 * Inequality O(n^2) approach
 * 
 * @author arun
 *
 */
public class UVA10003_4 {
			...
			for (int i = 2, iLen = m + 2; i < iLen; ++i) {
				for (int j = 0, jLen = iLen - i, l = i; j < jLen; ++j, ++l) {
					...
					if (i == 2) {
						...
						int startCut = cutIdx[c1][cuts[l - 1]], endCut = cutIdx[cuts[j + 1]][c2];
						//runs in amortized constant time
						for (int k = startCut; k <= endCut; ++k) {
							int curCost = dp[c1][cuts[k]] + dp[cuts[k]][c2] + cost;
							if (dp[c1][c2] >= curCost) {
								dp[c1][c2] = curCost;
								cutIdx[c1][c2] = k;
							}
