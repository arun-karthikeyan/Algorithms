...
static class IntegerArray implements Comparable<IntegerArray>{
	public int[] array;
...
public static void main(String[] args) throws Exception {
	...
	int n;
	while((n=readInt())!=0){
		AVLTreeBasic<IntegerArray> avl = new AVLTreeBasic<IntegerArray>();
		while(n-->0){
			int[] comb = new int[5];
			for(int i=0; i<5; ++i){
				comb[i] = readInt();
			}
			avl.insert(new IntegerArray(comb));
		}
		
		Iterator<Object[]> inOrderIterator = avl.inOrderIterator();
		int maxComb = (Integer)inOrderIterator.next()[1];
		int maxCombCount = maxComb;
		while(inOrderIterator.hasNext()){
			int currentVal = (Integer)inOrderIterator.next()[1];
			if(currentVal>maxComb){
				maxComb = currentVal;
				maxCombCount = currentVal;
			}else if(currentVal==maxComb){
				maxCombCount+=currentVal;
			}
		}
		pw.println(maxCombCount);
	}
